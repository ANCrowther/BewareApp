...
namespace Beware.ControllerModels {
    class ButtonLeftModel : IControllerModel {
        ...
        public void Draw(Vector2 position, Mode mode, bool isMobile = true, float orientation = 0) {
            Texture2D picture = (mode == Mode.Move) ? Controls.Arrow : Controls.Y;
            var gamepadState = GamePad.GetState(PlayerIndex.One);
            bool isActive = false;

            if (mode == Mode.Move) {
                isActive = ((gamepadState.GetButton() == Buttons.DPadLeft));
            }
            if (mode == Mode.Shoot) {
                isActive = ((gamepadState.GetButton() == Buttons.X));
            }
            ...
            if (mode == Mode.Move) {
                BewareGame.Instance._spriteBatch.Draw(picture, new Vector2(position.X - 75, position.Y), null, color, MathHelper.ToRadians(270.0f), new Vector2(picture.Width, picture.Height) / 2, 0.5f, 0, 0.0f);
            }
            if (mode == Mode.Shoot) {
